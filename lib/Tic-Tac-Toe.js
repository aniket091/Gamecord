...
class TicTacToe {
    ...
    async startGame() {
        ...
        const filter = (interaction) => interaction.user.id === this.opponent.id;
        const interaction = await askMsg.awaitMessageComponent({ filter, max: 1, time: 60000 }).catch(e => {

            for (let y = 0; y < askMsg.components[0].components.length; y++) {
              askMsg.components[0].components[y].disabled = true;
            }
            askMsg.embeds[0].description = this.options.timeEndMessage.replace('{opponent}', '<@!' + this.opponent.id + '>')
            .replace('{challenger}', '<@!' + this.message.author.id + '>');
        ...
        if (interaction.customId === 'reject') {
            ...
            askMsg.embeds[0].description = this.options.cancelMessage.replace('{opponent}', '<@!' + this.opponent.id + '>')
            .replace('{challenger}', '<@!' + this.message.author.id + '>');
            return askMsg.edit({ embeds: askMsg.embeds, components: askMsg.components });
